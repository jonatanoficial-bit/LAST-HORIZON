...
export function planetPosition(body,value){
  const jd=julianDate(value),T=(jd-2451545)/36525,elements=body.elements;
  ...
  const M=wrap180(L-peri)*DEG,omega=(peri-node)*DEG,Omega=node*DEG;
  let E=M;
  for(let i=0;i<12;i++)E-=((E-e*Math.sin(E)-M)/(1-e*Math.cos(E)));
  const xp=a*(Math.cos(E)-e),yp=a*Math.sqrt(1-e*e)*Math.sin(E);
  const cosW=Math.cos(omega),sinW=Math.sin(omega),cosO=Math.cos(Omega),sinO=Math.sin(Omega),cosI=Math.cos(I),sinI=Math.sin(I);
...
export function missionDate(state){
  ...
  const date=new Date(Date.UTC(2047,0,1,12));
  date.setUTCDate(date.getUTCDate()+(state.time?.earthDate||0));
  return date;
}
