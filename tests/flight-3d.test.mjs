...
test("immersive flight renderer exports an offline WebGL pipeline with visible free-look references",()=>{const source=fs.readFileSync(new URL("../src/ui/flight-3d.js",import.meta.url),"utf8");assert.equal(typeof createFlight3DRenderer,"function");assert.match(source,/uEarth/);assert.match(source,/uAtmosphere/);assert.match(source,/uAltitude/);assert.match(source,/uLook/);assert.match(source,/towerQ/);assert.match(source,/cloudTexture/);assert.match(source,/sunDisc/);assert.match(source,/skyZenith/);assert.match(source,/isContextLost/);assert.match(source,/rocketMesh/);assert.match(source,/earth-blue-marble-2048\.jpg/);assert.match(source,/look=\{yawDeg:0,pitchDeg:0\}/);assert.doesNotMatch(source,/https?:\/\//)});
